...
function toggleMessage(messageId) {
  const currentMessage = document.getElementById(`currentMessage-${messageId}`);
  const oldMessage = document.getElementById(`oldMessage-${messageId}`);
  const toggleButton = document.getElementById(`toggleButton-${messageId}`);

  if (oldMessage.style.display === "none") {
      oldMessage.style.display = "block";
      currentMessage.style.display = "none";
      toggleButton.innerHTML = "Show new message";
  } else {
      oldMessage.style.display = "none";
      currentMessage.style.display = "block";
      toggleButton.innerHTML = "Show old message";
  }
}
